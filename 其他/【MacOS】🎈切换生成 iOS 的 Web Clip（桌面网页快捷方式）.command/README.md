...
- 本自述文件对应脚本：`【MacOS】🎈切换生成 iOS 的 Web Clip（桌面网页快捷方式）.command`。
...
```shell
chmod +x './【MacOS】🎈切换生成 iOS 的 Web Clip（桌面网页快捷方式）.command'
'./【MacOS】🎈切换生成 iOS 的 Web Clip（桌面网页快捷方式）.command'
```
...
- 运行日志默认写入：`$TMPDIR/【MacOS】🎈切换生成 iOS 的 Web Clip（桌面网页快捷方式）.log`。
